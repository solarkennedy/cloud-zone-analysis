...
g = graphviz.Graph("Inter-Region Latency", engine='neato')
...
def region_to_label(region):
    return f"{human_region_name(region)}"
...
for row in c:
    ...
    for dest, value in row.items():
        ...
        g.edge(region_to_label(row['Region']), region_to_label(dest), len=str((int(value)/100)), label=f"{value}ms")
